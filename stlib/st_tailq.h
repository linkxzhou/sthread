...
template <class T> class CPP_TAILQ_ENTRY {
public:
  CPP_TAILQ_ENTRY()
      : tqe_next(NULL), tqe_prev(NULL), parent(NULL), remove_tag(true) {}
...
#define CPP_TAILQ_DECR(head)                                                   \
  (((head)->tqh_size <= 0) ? 0 : ((head)->tqh_size--))
...
#define CPP_TAILQ_FOREACH(var, head, field)                                    \
  for ((var) = CPP_TAILQ_FIRST(head); (var);                                   \
...
#define CPP_TAILQ_REMOVE(head, elm, field)                                     \
  do {                                                                         \
    if ((elm)->field.remove_tag)                                               \
      break;                                                                   \
    if ((CPP_TAILQ_NEXT((elm), field)) != NULL)                                \
      CPP_TAILQ_NEXT((elm), field)->field.tqe_prev = (elm)->field.tqe_prev;    \
    else                                                                       \
...
#define CPP_TAILQ_INSERT_TAIL(head, elm, field)                                \
  do {                                                                         \
...
#define CPP_TAILQ_POP(head, elm, field)                                        \
  do {                                                                         \
    elm = CPP_TAILQ_FIRST(head);                                               \
    CPP_TAILQ_REMOVE(head, elm, field);                                        \
  } while (0)

#define CPP_TAILQ_REMOVE_SELF(elm, field)                                      \
  do {                                                                         \
