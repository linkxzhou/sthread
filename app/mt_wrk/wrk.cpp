...
{
    char *url, **headers = malloc(argc * sizeof(char *));
    struct http_parser_url parts = {};

    if (parse_args(&c, &url, &parts, headers, argc, argv)) 
    {
        usage();
        exit(1);
    }
...
static int parse_args(config *_cg, char **url, struct http_parser_url *parts, 
    char **headers, int argc, char **argv) {
    char **header = headers;
    int c;

    memset(_cg, 0, sizeof(config));
    _cg->threads     = 2;
    _cg->connections = 10;
    _cg->duration    = 10;
