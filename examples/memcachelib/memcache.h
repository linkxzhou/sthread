...
typedef CPP_TAILQ_ENTRY<message> KeepMessageLink;
...
{
public:
    KeepMessageLink c_tqe;
    KeepMessageLink s_tqe;
    KeepMessageLink m_tqe;
...
static bool memcache_storage(message *r)
{
    switch (r->type) {
    case MSG_REQ_MC_SET:
    case MSG_REQ_MC_CAS:
...
static bool memcache_cas(message *r)
{
...
static bool memcache_retrieval(message *r)
{
    switch (r->type) 
    {
    case MSG_REQ_MC_GET:
...
static bool memcache_arithmetic(message *r)
{
    switch (r->type) 
    {
...
static bool memcache_delete(message *r)
{
...
static bool memcache_touch(message *r)
{
...
void memcache_parse_req(message *r)
{
    struct mbuf *b;
    uint8_t *p, *m;
    uint8_t ch;
    ...
    for (p = r->pos; p < b->last; p++) 
    {
        ...
        case SW_REQ_TYPE:
            if (ch == ' ' || ch == CR) 
            {
                ...
                {
                case 3: 
                    if (str4cmp(m, 'g', 'e', 't', ' ')) // get
                    {
                        r->type = MSG_REQ_MC_GET;
                        break;
                    }
                    if (str4cmp(m, 's', 'e', 't', ' ')) // set
                    {
                        r->type = MSG_REQ_MC_SET;
                        break;
                    }
                    if (str4cmp(m, 'a', 'd', 'd', ' ')) // add
                    {
                        r->type = MSG_REQ_MC_ADD;
                        break;
                    }
                    if (str4cmp(m, 'c', 'a', 's', ' ')) // cas
                    {
                ...
                case 4:
                    if (str4cmp(m, 'g', 'e', 't', 's')) // gets
                    {
                        r->type = MSG_REQ_MC_GETS;
                        break;
                    }
                    if (str4cmp(m, 'i', 'n', 'c', 'r')) // incr
                    {
                        r->type = MSG_REQ_MC_INCR;
                        break;
                    }
                    if (str4cmp(m, 'd', 'e', 'c', 'r')) // decr
                    {
                        r->type = MSG_REQ_MC_DECR;
                        break;
                    }
                    if (str4cmp(m, 'q', 'u', 'i', 't')) // quit
                    {
                ...
                case 6:
                    if (str6cmp(m, 'a', 'p', 'p', 'e', 'n', 'd')) // append
                    {
                        r->type = MSG_REQ_MC_APPEND;
                        break;
                    }
                    if (str6cmp(m, 'd', 'e', 'l', 'e', 't', 'e')) // delete
                    {
                ...
                case 7:
                    if (str7cmp(m, 'p', 'r', 'e', 'p', 'e', 'n', 'd')) // prepend
                    {
                        r->type = MSG_REQ_MC_PREPEND;
                        break;
                    }
                    if (str7cmp(m, 'r', 'e', 'p', 'l', 'a', 'c', 'e')) // replace
                    {
...
static rstatus_t memcache_append_key(struct msg *r, uint8_t *key, uint32_t keylen)
{
    struct mbuf *mbuf;
    struct keypos *kpos;
