...
/* 用途：一个 fd 上的可读/可写/挂起事件载体，虚回调 EvInput/EvOutput/EvHangup
 * 供派生覆盖。 线程模型：仅在创建它的 OS 线程的 StEventSchedule
 * 内使用；勿跨线程传递。 所有权：通常由 UtilPtrPool<StEventItem>
 * 池化；ClearItem/UtilPtrPoolFree 归还。 */
...
public:
  ...
  virtual ~StThreadItem() { this->Reset(); }
  ...
  inline bool HasFlag(eThreadFlag flag) { return (m_flag_ & flag); }

  inline void SetType(eThreadType type) { m_type_ = type; }

  inline eThreadType GetType() { return m_type_; }
  ...
  inline Stack *GetStack() { return m_stack_; }

  inline void ResetFdSet() { CPP_TAILQ_INIT(&m_fdset_); }

  // 获取fdset的列表
  inline StEventItemQueue &GetFdSet(void) { return m_fdset_; }
  ...
  // 判断类型
  inline bool IsDaemon(void) { return (eDAEMON == m_type_); }
