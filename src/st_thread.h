...
public:
    ...
    inline StThreadItem* GetActiveThread()
    {
    ...
    void SwitchThread(StThreadItem *rthread, 
    ...
    StThread* CreateThread(StClosure *closure, 
        bool runable = true);
...
{
public:
    typedef StEventItem* StEventItemPointer;

    StEventSchedule(int32_t max_num = 1024) : 
        m_maxfd_(65535), 
        m_apistate_(new StApiState()), 
        m_array_(NULL), 
    ...
    { 
        ...
        st_safe_delete_array(m_array_);
    }

    int32_t Init(int32_t max_num);

    bool Schedule(StThreadItem *thread, 
        StEventItemQueue *item_list,
        StEventItem *item,
    ...
    {
        return IsValidFd(fd) ? m_array_[fd] : NULL;
    }
